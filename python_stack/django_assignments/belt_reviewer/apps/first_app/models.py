      ...
      def register(self, postData, sessionData, request):
            ...
            if not EMAIL_REGEX.match(sessionData['email']):
                  ...
                  if User.objects.filter(email=sessionData['email']).exists():
                        ...
                        User.objects.create(first_name = sessionData['first_name'], last_name= sessionData['last_name'],alias= sessionData['alias'], email = sessionData['email'],  password=hashed_pw, salt = salt)
