...
@app.route('/')   
def hello_flask():                        # The "@" symbol designates a "decorator" which attaches the
  return render_template('index.html')
                                          # whenever we send a request to localhost:5000/ we will run
                                          # the following "hello_world" function.
def hello_world():
  return render_template('template_test.html', name="Jay")    # Render the template and return it!
